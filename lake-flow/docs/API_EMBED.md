# API String embedding (Embed)

API to embed a text string using the same model as Semantic Search (`sentence-transformers/all-MiniLM-L6-v2`). Returned vector is normalized, suitable for cosine similarity.
...
|--------|------|----------|--------|
| `text` | string | Yes | String to embed. Must not be empty. |
...
|--------|------|--------|
| `text` | string | Echo of sent string. |
| `vector` | array of float | Embedding vector (normalized). |
